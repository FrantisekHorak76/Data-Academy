 ...
 * Tabulka avg_product_difference = meziroèní procentuální rozdílu jednotlivých potravin ve sledovaném období
 * Tabulka avg_percentage = seøazení jednotlivých potravin podle prùmìrného meziroèního procentuálního nárùstu, nebo poklesu ceny za celé období.*/
...
/* Vytvoøení výsledné odpovìdi na otázku è.3 */
SELECT 
	*
FROM avg_percentage 
WHERE  
